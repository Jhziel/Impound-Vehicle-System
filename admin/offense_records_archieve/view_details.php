...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT r.*,d.license_id_no, d.name as driver from `ticket_archive` r inner join `drivers_list` d on r.driver_id = d.id where r.id = '{$_GET['id']}' ");
    if($conn->error){
        echo $conn->error ."\n";
        echo "SELECT r.*,d.license_id_no, d.name as driver from `ticket_archive` r inner join `drivers_list` on r.driver_id = d.id where r.id = '{$_GET['id']}' ";
    }
            ...
            <tr>
                <td colspan='2'>
                    ...
                    <div class="d-flex w-max-100">
                        ...
                        <p class=" border-bottom border-dark px-4"><b><?php echo ($status == 1) ? "Paid" : "Pending" ?></b></p>
                    </div>
                </td>
            </tr>
        </table>
