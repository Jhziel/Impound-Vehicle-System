...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    $qry = $conn->query("SELECT * from `enforcers_list` where id = '{$_GET['id']}' ");
    $qry2 = $conn->query("SELECT * from `enforcers_meta` where enforcers_id = '{$_GET['id']}' ");
    if ($qry->num_rows > 0) {
...
<div class="card card-outline card-info">
    <div class="card-header">
        <h3 class="card-title"><?php echo isset($id) ? "Update " : "Create New " ?> enfrocer</h3>
        <div class="card-tools">

            <a href="?page=enforcers" class="btn btn-outline-secondary mr-2">
                <i class="fas fa-arrow-left"></i> Back
            </a>
            <button class="btn btn-danger mr-2" id="clear-all">
                <i class="fas fa-times"></i> Clear All
            </button>
            <button class="btn btn-primary" form="enforcer-form">
    ...
    <div class="card-body">
        <form action="" id="enforcer-form">

            <input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        <label for="license_id_no" class="control-label">Employee ID No.</label>
                        <input type="text" maxlength="50" class="form-control form" autocomplete="off" required name="employee_id_no" value="<?php echo isset($employee_id_no) ? $employee_id_no : '' ?>">
                    </div>
                    <div class="form-group">
                ...
                <div class="col-6">
                    <div class="form-group">
                        ...
                        <select name="civil_status" id="civil_status" class="custom-select select2" required>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#clear-all').click(function() {
            $('#enforcer-form')[0].reset(); // Reset the form
          
        });

        $('#enforcer-form').submit(function(e) {
            e.preventDefault();
            var _this = $(this)
            $('.err-msg').remove();
            start_loader();
            $.ajax({
                url: _base_url_ + "classes/Master.php?f=save_enforcer",
