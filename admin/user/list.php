			...
			<div class="container-fluid">
				<table class="table table-bordered table-stripped">
					...
					<tbody>
						<?php
						$i = 1;
						$qry = $conn->query("SELECT *,concat(firstname,' ',lastname) as name from `users` where id != '1' and id != '{$_settings->userdata('id')}' and type!='3' order by  unix_timestamp(date_added) desc ");
						...
						?>
							<tr>
								...
								<td><?php echo ($row['type'] == 1) ? 'Administrator' : (($row['type'] == 2) ? 'Staff' : 'Driver'); ?></td>
							
								<td class="d-flex">
									<a class="btn btn-primary  mr-1" style="padding: 6px 12px; width:  71.7188px;" href="?page=user/manage_user&id=<?php echo $row['id'] ?>"> Edit</a>
