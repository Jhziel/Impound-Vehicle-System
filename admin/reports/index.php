...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title font-weight-bold">Reports of Unsettled Violations</h3>
        ...
        <div class="">
            <div class="row">
                ...
                <div class="col-2 mt-2">
                    <lable class="control-label text-bold" for="status">Status</lable>
            ...
            <div class="container-fluid" id="print_out">
                <table class="table table-hover table-stripped">
                    ...
                    <tbody>
                        <?php
                        $i = 1;
                        $qry = $conn->query("SELECT r.*,d.license_id_no FROM `offense_list` r inner join `drivers_list` d on r.driver_id = d.id where r.status= '$status' and date(r.date_created) between '{$date_start}' and '{$date_end}' order by unix_timestamp(r.date_created) desc ");
...
<script>
    $(document).ready(function() {
        $('#filter').click(function() {
            location.replace("./?page=reports&date_start=" + ($('#date_start').val()) + "&date_end=" + ($('#date_end').val()) + "&status=" + ($('#status').val()));
        })
